...
using UnityEngine.InputSystem;
	...
	private Vector2 lastPos;
		...
		void Start()
		{
			...
			if (EditorPrefs.HasKey("PhysicsEditMode"))
			{
				...
				else
				{
					...
					Font arial = (Font)AssetDatabase.LoadAssetAtPath(EditorPrefs.GetString("RelativePath") + "arial.ttf", typeof(Font));
		...
		void Update()
		{
			...
			if(Keyboard.current[Key.LeftAlt].wasReleasedThisFrame){
			...
			if(Mouse.current.scroll.ReadValue().y != 0){
				if (SpawnPoint.gameObject.activeSelf) {
					spawnOffcetY += Mouse.current.scroll.ReadValue().y * 0.001f;
					...
					if (Keyboard.current[Key.LeftShift].isPressed) {
						transform.Translate (0, 0, Mouse.current.scroll.ReadValue().y * Time.deltaTime);
					} else {
						transform.Translate (0, 0, Mouse.current.scroll.ReadValue().y * Time.deltaTime * 0.5f);
					}
				}
			}
			...
			if(Mouse.current.middleButton.isPressed){
				Vector2 delta = Mouse.current.position.ReadValue() - lastPos;
				transform.Translate (-delta.x * 0.3f * Time.deltaTime, -delta.y * 0.3f * Time.deltaTime, 0);
				lastPos = Mouse.current.position.ReadValue();
			}

			if(Mouse.current.rightButton.isPressed && !Keyboard.current[Key.LeftAlt].isPressed){
			...
			transform.Translate(Vector3.forward * 3 * boost * verticalAxis * Time.deltaTime);
			transform.Translate(Vector3.right * 3 * boost * horizontalAxis * Time.deltaTime);
			}
			...
			if (Keyboard.current[Key.LeftCtrl].isPressed) {
				...
				if (Physics.Raycast (ray, out hit, 1000, layermask, QueryTriggerInteraction.Ignore)) {
					...
					Cursor.SetCursor (cursorEmpty, Vector2.zero, CursorMode.ForceSoftware);
					SpawnPoint.position = hit.point;
				}

				if (Mouse.current.rightButton.wasPressedThisFrame && SpawnPoint.gameObject.activeSelf) {
					SpawnObjectRuntime ();
